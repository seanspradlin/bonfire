 ...
 */
function wrapSingleDocument(doc: { id: string; title: string; content: string }): string {
...
async function runQueryKnowledgeBase(
	...
	if (results.length === 0) {
		return {
			text: JSON.stringify({ results: [], note: 'No relevant documents found.' }),
			sources: []
		};
	}
	...
	return {
		...
		sources: formatted.map((r) => ({ id: r.parentId ?? r.id, title: r.title }))
	};
}
...
async function runSearchDocuments(
	...
	if (!query) {
		return { text: JSON.stringify({ error: 'query is required' }), sources: [] };
	}
	...
	if (signal?.aborted) throw new DOMException('Aborted', 'AbortError');
	const candidates = await repo.search({ embedding, limit: candidateLimit, tag });
	...
	if (results.length === 0) {
		return {
			text: JSON.stringify({ results: [], note: 'No matching documents found.' }),
			sources: []
		};
	}
...
async function runGetDocument(
	...
	const wrapped = wrapSingleDocument({ id: doc.id, title: doc.title, content: doc.content });
