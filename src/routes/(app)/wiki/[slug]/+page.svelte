...
<div class="mx-auto max-w-3xl px-8 py-8">
	...
	{#if toc.length > 0}
		<div class="mb-6 rounded-xl border border-border bg-bg-card px-5 py-4">
			<h2 class="mb-3 text-xs font-semibold tracking-wider text-text-muted uppercase">Contents</h2>
			<nav>
				<ol class="space-y-1">
					{#each toc as entry, i (entry.slug)}
						<li style="padding-left: {(entry.level - Math.min(...toc.map((e) => e.level))) * 1}rem">
							<a
								href="#{entry.slug}"
