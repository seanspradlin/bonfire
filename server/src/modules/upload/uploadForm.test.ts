...
describe("parseOptionalId", () => {
	...
	it("returns the id string when valid", () => {
		expect(parseOptionalId(formDataWith({ id: "my-doc-id" }))).toBe(
			"my-doc-id",
		);
	});
	...
	it("returns null when the id contains the reserved :chunk: namespace", () => {
		expect(
			parseOptionalId(formDataWith({ id: "parent:chunk:0001" })),
		).toBeNull();
	});

	it("accepts an id that contains 'chunk' but not ':chunk:'", () => {
		expect(parseOptionalId(formDataWith({ id: "chunk-of-text" }))).toBe(
			"chunk-of-text",
		);
	});
...
describe("parseOptionalDate", () => {
	...
	it("returns a UTC ISO string for a valid ISO 8601 datetime", () => {
		const result = parseOptionalDate(
			formDataWith({ date: "2026-04-20T12:00:00.000Z" }),
		);
	...
	it("returns null for a datetime with a numeric timezone offset (Zod requires Z suffix)", () => {
		// Zod's z.string().datetime() rejects offset notation like +05:00 by default.
		// Callers must submit UTC datetimes with a Z suffix.
		const result = parseOptionalDate(
			formDataWith({ date: "2026-04-20T12:00:00.000+05:00" }),
		);
	...
	it("returns null for a date-only string (no time component)", () => {
		// Zod's z.string().datetime() requires a time component
...
describe("parseOptionalTags", () => {
	...
	it("splits a comma-separated string into an array", () => {
		expect(parseOptionalTags(formDataWith({ tags: "a,b,c" }))).toEqual([
			"a",
			"b",
			"c",
		]);
	...
	it("trims whitespace from each tag", () => {
		expect(parseOptionalTags(formDataWith({ tags: " foo , bar " }))).toEqual([
			"foo",
			"bar",
		]);
	});
	...
	it("filters out empty strings produced by trailing commas", () => {
		expect(parseOptionalTags(formDataWith({ tags: "a,,b," }))).toEqual([
			"a",
			"b",
		]);
	});
...
describe("parseOptionalString", () => {
	...
	it("returns the string value when the field is present", () => {
		expect(
			parseOptionalString(formDataWith({ context: "some hint" }), "context"),
		).toBe("some hint");
	});

	it("trims surrounding whitespace", () => {
		expect(
			parseOptionalString(formDataWith({ context: "  padded  " }), "context"),
		).toBe("padded");
	});

	it("returns undefined when the field is whitespace only", () => {
		expect(
			parseOptionalString(formDataWith({ context: "   " }), "context"),
		).toBeUndefined();
	});
});
