...
describe("chunkMarkdown — splitting", () => {
	it("splits a long document into multiple chunks", () => {
		// Generate text that is clearly over the default 500-token limit
	...
	it("preserves all content across all chunks (no character loss)", () => {
		// The chunker only guarantees that the total character count is preserved —
		// it does not guarantee word-boundary splits (the last-resort hard-character
		// split is explicitly "deliberately crude, no word boundary").
		const words = Array.from({ length: 600 }, (_, i) => `word${i}`);
		const content = words.join(" ");
		const chunks = chunkMarkdown(content, 100);
		const totalChunkChars = chunks
	...
	it("attaches a headingContext to chunks split away from a heading", () => {
		...
		// At least one chunk after the first should carry the heading context
	...
	it("does not attach a headingContext to a chunk that starts with its own heading", () => {
		...
		// Chunks that begin with a heading should not have an external context
		for (const chunk of chunks) {
	...
	it("falls back to hard character splitting when no separator works", () => {
		// A single unbreakable token blob (no spaces, no periods)
...
describe("applyChunkOverlap", () => {
	...
	it("returns the same array for a single text", () => {
		expect(applyChunkOverlap(["only one"], CHUNK_OVERLAP_TOKENS)).toEqual([
			"only one",
		]);
	});
